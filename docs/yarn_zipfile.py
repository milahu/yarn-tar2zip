...
https://github.com/python/cpython/raw/202efe1a3bcd499f3bf17bd953c6d36d47747e78/Lib/zipfile.py

i have used "7z l" to compare results

python3 yarn_zipfile.py -c actual.zip \
  node_modules/ node_modules/accepts/ node_modules/accepts/package.json node_modules/accepts/README.md node_modules/accepts/LICENSE node_modules/accepts/index.js node_modules/accepts/HISTORY.md \
  && diff -u20 <(7z l -slt actual.zip) <(7z l -slt expected.zip)

the order of files is the same as in the tar archive

expected.zip was extracted by adding this code to yarn

le is the checksum from yarn.lock

// ...
let le=!u.skipIntegrityCheck||!r?`${this.cacheKey}/${await NP(we)}`:r;
//
if (le == "10c0/a62b5b81daa74afb60295e616982b35e2bd183aaf263a35e235f3736058c37d6f5f6b9ce435e46099e4105ecdbe205b120a3b771afd11f3d2747a9522a14e130") {
  // we = /run/user/1000/xfs-06bfd4d8/archive.zip
  console.error(`file ${we}`);
  console.error(`filehash ${await NP(we)}`);
  console.error(`actualChecksum ${le}`);
  const cp = require("child_process");
  console.error(`$ cp ${we} /run/user/1000/yarn-archive.zip`);
  console.error(cp.spawnSync("cp", [we, "/run/user/1000/yarn-archive.zip"], {encoding: "utf8"}).stdout);
}
//
if(Ee!==le)throw new zt(18,"The remote archive doesn't match the local checksum
...
def date_time() -> Union[time.struct_time, Tuple[int, int, int, int, int, int]]:
    ...
    """
    return (1984, 6, 22, 21, 50, 0) # 1984-06-22 21:50:00 = 1980 + 456789000 seconds
...
def main():
    ...
    with ReproducibleZipFile(output_path, "w") as zp:
        ...
        for path in sys.argv[2:]: # dont sort
            if os.path.isdir(path):
